...
def main(fname):
    ...
    plt.semilogx(xaxis, g_trai[:, 2], label=r'G (train)')
    plt.semilogx(xaxis, g_test[:, 2], label=r'G (test)')
    plt.semilogx(xaxis, d_trai[:, 2], label=r'D (train)')
    plt.semilogx(xaxis, d_test[:, 2], label=r'D (test)')
    plt.xlabel(r'Epoch')
    plt.ylabel(r'Loss')
    plt.xlim([0, 50])
    plt.legend()
